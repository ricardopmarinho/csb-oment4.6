...
#include "inet/routing/aodvbr/AODVBRRouting.h"
...
Define_Module(AODVBRRouting);

void AODVBRRouting::initialize(int stage)
{
...
void AODVBRRouting::handleMessage(cMessage *msg)
{
...
INetfilter::IHook::Result AODVBRRouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
...
AODVBRRouting::AODVBRRouting()
{
}

bool AODVBRRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AODVBRRouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address AODVBRRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AODVBRRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void AODVBRRouting::sendRREQ(AODVRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVBRRouting::sendRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREQ *AODVBRRouting::createRREQ(const L3Address& destAddr)
{
...
AODVRREP *AODVBRRouting::createRREP(AODVRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
AODVRREP *AODVBRRouting::createGratuitousRREP(AODVRREQ *rreq, IRoute *originatorRoute)
{
...
void AODVBRRouting::handleRREP(AODVRREP *rrep, const L3Address& sourceAddr)
{
...
void AODVBRRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AODVBRRouting::sendAODVPacket(AODVControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AODVBRRouting::handleRREQ(AODVRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        /*Checking if the host can send its RREP message:
         * i) check in its routing table if a route to the source already exist
         * ii) if exists, compares the hop count from the route it is creating with the hop count
         * of the route it has stored
         * iii) in case that the hop count from the route it is creating is bigger than the hop count
         * of the route it has stored, it does not send the RREP message
         * */
...
IRoute *AODVBRRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AODVBRRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
...
void AODVBRRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
AODVRERR *AODVBRRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void AODVBRRouting::handleRERR(AODVRERR *rerr, const L3Address& sourceAddr)
{
...
bool AODVBRRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void AODVBRRouting::handleWaitForRREP(WaitForRREP *rrepTimer)
{
...
void AODVBRRouting::forwardRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVBRRouting::forwardRREQ(AODVRREQ *rreq, unsigned int timeToLive)
{
...
void AODVBRRouting::completeRouteDiscovery(const L3Address& target)
{
...
void AODVBRRouting::sendGRREP(AODVRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREP *AODVBRRouting::createHelloMessage()
{
...
void AODVBRRouting::sendHelloMessagesIfNeeded()
{
...
void AODVBRRouting::handleHelloMessage(AODVRREP *helloMessage)
{
...
void AODVBRRouting::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void AODVBRRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AODVBRRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
...
void AODVBRRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void AODVBRRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AODVBRRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AODVRouteData *routeData = check_and_cast<AODVRouteData *>(route->getProtocolData());
...
AODVRREPACK *AODVBRRouting::createRREPACK()
{
...
void AODVBRRouting::sendRREPACK(AODVRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AODVBRRouting::handleRREPACK(AODVRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void AODVBRRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AODVBRRouting::handleBlackListTimer()
{
