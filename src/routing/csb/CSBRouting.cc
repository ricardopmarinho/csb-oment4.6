...
#include "inet/routing/csb/CSBRouting.h"
...
Define_Module(CSBRouting);

void CSBRouting::initialize(int stage)
{
    rreqSentSignal = registerSignal("rreqSent");
    rrepSentSignal = registerSignal("rrepSent");
    rreqRecSignal = registerSignal("rreqRec");
    rreq1RecSignal = registerSignal("rreq1Rec");
    rrepRecSignal = registerSignal("rrepRec");
    rrep1RecSignal = registerSignal("rrep1Rec");
    rrep2RecSignal = registerSignal("rrep2Rec");
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        numHosts = host->getParentModule()->par("numHosts").doubleValue();
        numSocialNetworks = par("numSocialNetworks");
        initializeSocianNetworks();
...
void CSBRouting::handleMessage(cMessage *msg)
{
   ...
   if (!isOperational) {
    ...
    if (msg->isSelfMessage()) {
        if (dynamic_cast<WaitForRREP *>(msg))
            ...
            rreqCount = rerrCount = 0;
            scheduleAt(simTime() + 1, counterTimer);
            CONTACTINFO *contactMsg = createCONTACTINFO();
            sendCSBPacket(contactMsg,addressType->getBroadcastAddress(),1,0);
        ...
        if(strncmp("ICMP",msg->getName(),strlen("ICMP"))){
            UDPPacket *udpPacket = check_and_cast<UDPPacket *>(msg);
            CSBControlPacket *ctrlPacket = check_and_cast<CSBControlPacket *>(udpPacket->decapsulate());
            ...
            switch (ctrlPacket->getPacketType()) {
                case RREQ:
                    rreqRec++;
                    emit(rreqRecSignal,rreqRec);
                    handleRREQ(check_and_cast<CSBRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                    break;

                case RREP:
                    rrepRec++;
                    emit(rrepRecSignal,rrepRec);
                    handleRREP(check_and_cast<CSBRREP *>(ctrlPacket), sourceAddr);
                    break;

                case RERR:
                    handleRERR(check_and_cast<CSBRERR *>(ctrlPacket), sourceAddr);
                    break;

                case RREPACK:
                    handleRREPACK(check_and_cast<CSBRREPACK *>(ctrlPacket), sourceAddr);
                    break;

                case ContactInfo:
                    handleCONTACTINFO(check_and_cast<CONTACTINFO *>(ctrlPacket));
                    break;

                default:
                    throw cRuntimeError("CSB Control Packet arrived with undefined packet type: %d", ctrlPacket->getPacketType());
            }
            delete udpPacket;
        }
    }
...
void CSBRouting::initializeSocianNetworks(){
    int i;
    for(i=0;i<numSocialNetworks;i++){
        unsigned int friendsNum = intuniform(1,16);
        SocialNetwork* newSN = new SocialNetwork(i,friendsNum);
        SN.push_back(newSN);
    }
}


INetfilter::IHook::Result CSBRouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        CSBRouteData *routeData = route ? dynamic_cast<CSBRouteData *>(route->getProtocolData()) : nullptr;
...
CSBRouting::CSBRouting()
{
}

bool CSBRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void CSBRouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
    ...
    //checking if the host received some information about target location. If not, return.
    CSBRREQ *rreq = createRREQ(target);
    addressToRreqRetries[target] = 0;
    sendRREQ(rreq, addressType->getBroadcastAddress(), timeToLive);
}

L3Address CSBRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void CSBRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void CSBRouting::sendRREQ(CSBRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendCSBPacket(rreq, destAddr, timeToLive, jitterPar->doubleValue());
    rreqCount++;
    rreqSent++;
    emit(rreqSentSignal,rreqSent);
}

void CSBRouting::sendRREP(CSBRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    CSBRouteData *destRouteData = check_and_cast<CSBRouteData *>(destRoute->getProtocolData());
    ...
    sendCSBPacket(rrep, nextHop, timeToLive, 0);
    rrepSent++;
    emit(rrepSentSignal,rrepSent);
}

CSBRREQ *CSBRouting::createRREQ(const L3Address& destAddr)
{
    CSBRREQ *rreqPacket = new CSBRREQ("CSB-RREQ");
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        CSBRouteData *routeData = check_and_cast<CSBRouteData *>(lastKnownRoute->getProtocolData());
...
CSBRREP *CSBRouting::createRREP(CSBRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    CSBRREP *rrep = new CSBRREP("CSB-RREP");
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // node is itself the requested destination
        ...
        CSBRouteData *destRouteData = check_and_cast<CSBRouteData *>(destRoute->getProtocolData());
        CSBRouteData *originatorRouteData = check_and_cast<CSBRouteData *>(originatorRoute->getProtocolData());
...
CSBRREP *CSBRouting::createGratuitousRREP(CSBRREQ *rreq, IRoute *originatorRoute)
{
    ASSERT(originatorRoute != nullptr);
    CSBRREP *grrep = new CSBRREP("CSB-GRREP");
    CSBRouteData *routeData = check_and_cast<CSBRouteData *>(originatorRoute->getProtocolData());
...
CONTACTINFO *CSBRouting::createCONTACTINFO(){
    char msgName[20];
    sprintf(msgName,"CONTACTINFO");
    CONTACTINFO* contactMsg = new CONTACTINFO(msgName);
    contactMsg->setHostIndex(host->getIndex());
    contactMsg->setSourceAddr(getSelfIPAddress());
    return contactMsg;
}

void CSBRouting::handleRREP(CSBRREP *rrep, const L3Address& sourceAddr)
{
    EV_INFO << "CSB Route Reply arrived with source addr: " << sourceAddr << " originator addr: " << rrep->getOriginatorAddr()
            << " destination addr: " << rrep->getDestAddr() << endl;

    double newRouteWeight = 0;
    ...
    CSBRouteData *destRouteData = nullptr;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<CSBRouteData *>(destRoute->getProtocolData());
        ...
        //      known value is valid, or
            ...
            newRouteWeight = calcRouteWeight(sourceAddr);
            routeWeight = calcRouteWeight(destRoute->getNextHopAsGeneric());
            if (destSeqNum == destRouteData->getDestSeqNum() && !destRouteData->isActive()) {
                updateRoutingTable(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
            }
            // (iv) the sequence numbers are the same, and the New Route Weight is
            //      bigger than the route weight in route table entry.
            else if (destSeqNum == destRouteData->getDestSeqNum() && (newRouteWeight > routeWeight)) {
                updateRoutingTable(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
            }
        }
    }
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (rrep->getAckRequiredFlag()) {
                    CSBRREPACK *rrepACK = createRREPACK();
                ...
                if (nextHopToDestRoute && nextHopToDestRoute->getSource() == this) {
                    CSBRouteData *nextHopToDestRouteData = check_and_cast<CSBRouteData *>(nextHopToDestRoute->getProtocolData());
                    nextHopToDestRouteData->addPrecursor(originatorRoute->getNextHopAsGeneric());
                }
                CSBRREP *outgoingRREP = rrep->dup();
                forwardRREP(outgoingRREP, originatorRoute->getNextHopAsGeneric(), 100);
                rrep2Rec++;
                emit(rrep2RecSignal,rrep2Rec);
            }
        }
...
void CSBRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    EV_DETAIL << "Updating existing route: " << route << endl;

    double routeWeight = 0;

    routeWeight = calcRouteWeight(nextHop);

    route->setNextHop(nextHop);
    route->setMetric(hopCount);

    CSBRouteData *routingData = check_and_cast<CSBRouteData *>(route->getProtocolData());
    ...
    routingData->setWeight(routeWeight);
...
void CSBRouting::sendCSBPacket(CSBControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
    ...
    udpPacket->setSourcePort(csbUDPPort);
    udpPacket->setDestinationPort(csbUDPPort);
...
void CSBRouting::handleRREQ(CSBRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{

    EV_INFO << "CSB Route Request arrived with source addr: " << sourceAddr << " originator addr: " << rreq->getOriginatorAddr()
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        CSBRouteData *routeData = check_and_cast<CSBRouteData *>(reverseRoute->getProtocolData());
    ...
    CSBRouteData *destRouteData = destRoute ? dynamic_cast<CSBRouteData *>(destRoute->getProtocolData()) : nullptr;

    // check (i)
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        EV_INFO << "I am the destination node for which the route was requested" << endl;

        IRoute *hopRoute = routingTable->findBestMatchingRoute(sourceAddr);
        if(hopRoute != nullptr){
            if( rreq->getHopCount() > hopRoute->getMetric() ){
              return;
            }
        }

        // create RREP
        CSBRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
        ...
        destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag()) {
            ...
            CSBRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        CSBRREQ *outgoingRREQ = rreq->dup();
        forwardRREQ(outgoingRREQ, timeToLive);
    }
    else
        EV_WARN << "Can't forward the RREQ because of its small (<= 1) TTL: " << timeToLive << " or the CSB reboot has not completed yet" << endl;

    delete rreq;
}


IRoute *CSBRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    CSBRouteData *newProtocolData = new CSBRouteData();
    ...
    newProtocolData->setWeight(routeWeight);
    ...
    newRoute->setSourceType(IRoute::CSB);
...
void CSBRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
...
void CSBRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    CSBRouteData *unreachableRouteData = check_and_cast<CSBRouteData *>(unreachableRoute->getProtocolData());
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getNextHopAsGeneric() == unreachableAddr) {
            CSBRouteData *routeData = check_and_cast<CSBRouteData *>(route->getProtocolData());
    ...
    CSBRERR *rerr = createRERR(unreachableNodes);
    rerrCount++;

    // broadcast
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendCSBPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());
}

CSBRERR *CSBRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    CSBRERR *rerr = new CSBRERR("CSB-RERR");
...
Coord CSBRouting::createReceiverCoord(double x,double y,long speed,simtime_t time, unsigned int type){
    Coord receiverCoord(Coord::ZERO);
    simtime_t now = simTime();
    double calcTime = SIMTIME_DBL(now-time);
    switch(type){
    case 1:
        receiverCoord.x = x+speed*calcTime;
        receiverCoord.y = y+speed*calcTime;
        receiverCoord.z = 0;
        break;
    case 2:
        receiverCoord.x = x+speed*calcTime;
    ...
    return receiverCoord;
}

void CSBRouting::handleRERR(CSBRERR *rerr, const L3Address& sourceAddr)
{
    EV_INFO << "CSB Route Error arrived with source addr: " << sourceAddr << endl;
        ...
        if (route->getNextHopAsGeneric() == sourceAddr) {
            for (unsigned int j = 0; j < unreachableArraySize; j++) {
                if (route->getDestinationAsGeneric() == rerr->getUnreachableNodes(j).addr) {
                    ...
                    routeData->setIsActive(false);    // it means invalid, see 3. CSB Terminology p.3. in RFC 3561
                    routeData->setLifeTime(simTime() + deletePeriod);
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        CSBRERR *newRERR = createRERR(unreachableNeighbors);
        sendCSBPacket(newRERR, addressType->getBroadcastAddress(), 1, 0);
        rerrCount++;
    }
    delete rerr;
}

bool CSBRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void CSBRouting::handleWaitForRREP(WaitForRREP *rrepTimer)
{
    ...
    CSBRREQ *rreq = createRREQ(destAddr);
...
void CSBRouting::forwardRREP(CSBRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendCSBPacket(rrep, destAddr, 100, jitterPar->doubleValue());
}

void CSBRouting::forwardRREQ(CSBRREQ *rreq, unsigned int timeToLive)
{
    EV_INFO << "Forwarding the Route Request message with TTL= " << timeToLive << endl;
    sendCSBPacket(rreq, addressType->getBroadcastAddress(), timeToLive, jitterPar->doubleValue());
}

void CSBRouting::completeRouteDiscovery(const L3Address& target)
{
...
void CSBRouting::sendGRREP(CSBRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendCSBPacket(grrep, nextHop, timeToLive, 0);
}

CSBRREP *CSBRouting::createHelloMessage()
{
    ...
    CSBRREP *helloMessage = new CSBRREP("CSB-HelloMsg");
...
void CSBRouting::sendHelloMessagesIfNeeded()
{
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval)) {
        ...
        CSBRREP *helloMessage = createHelloMessage();
        sendCSBPacket(helloMessage, addressType->getBroadcastAddress(), 1, 0);
    }

    scheduleAt(simTime() + helloInterval - periodicJitter->doubleValue(), helloMsgTimer);
}

void CSBRouting::handleHelloMessage(CSBRREP *helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        CSBRouteData *routeData = check_and_cast<CSBRouteData *>(routeHelloOriginator->getProtocolData());
...
void CSBRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result CSBRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
    ...
    CSBRouteData *routeDestData = routeDest ? dynamic_cast<CSBRouteData *>(routeDest->getProtocolData()) : nullptr;
...
void CSBRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IRoute *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    CSBRouteData *unreachableRouteData = unreachableRoute ? dynamic_cast<CSBRouteData *>(unreachableRoute->getProtocolData()) : nullptr;
    ...
    rerrCount++;
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendCSBPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());    // TODO: unicast if there exists a route to the source
}

void CSBRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool CSBRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        CSBRouteData *routeData = check_and_cast<CSBRouteData *>(route->getProtocolData());
...
CSBRREPACK *CSBRouting::createRREPACK()
{
    CSBRREPACK *rrepACK = new CSBRREPACK("CSB-RREPACK");
    rrepACK->setPacketType(RREPACK);
    rrepACK->setByteLength(2);
    return rrepACK;
}

void CSBRouting::sendRREPACK(CSBRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendCSBPacket(rrepACK, destAddr, 100, 0);
}

void CSBRouting::handleRREPACK(CSBRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void CSBRouting::handleCONTACTINFO(CONTACTINFO *contactInfoMsg){
    int hostIndex = contactInfoMsg->getHostIndex();
    if(contactInfo[hostIndex] == nullptr){
        contactInfo[hostIndex] = new CSBContactInfo(SIMTIME_ZERO,simTime());
    }else{
        updateContactInfo(contactInfo[hostIndex],contactInfoMsg->getSourceAddr());
    }
    delete contactInfoMsg;
}

void CSBRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void CSBRouting::initializeContactTable(){
    for(int i = 0; i < numHosts; i++)
        contactInfo.push_back(nullptr);
}

void CSBRouting::updateContactInfo(CSBContactInfo *cInfo, L3Address& sourceAddr){
    IRoute *source = routingTable->findBestMatchingRoute(sourceAddr);
    if(source == nullptr){
        cInfo->setContactDuration(SIMTIME_ZERO);
        cInfo->setLastContactTime(simTime());
        cInfo->incrementContactNumber();
        return;
    }
    cInfo->setContactDuration(simTime()-cInfo->getLastContactTime());
    cInfo->setLastContactTime(simTime());
}


double CSBRouting::calcRouteWeight(const L3Address& nextHop){
    int i;
    double sNetworkRouteWeight = 0;
    double contactWeight = 0;
    double routeWeight = 0;
    for(i = 0; i < numSocialNetworks; i++){
        sNetworkRouteWeight += SN[i]->networkWeight(nextHop);
    }
    sNetworkRouteWeight *= 1;

    contactWeight = calcContactWeight(nextHop);
    contactWeight *= 0;

    routeWeight = contactWeight+sNetworkRouteWeight;

    return routeWeight;
}

double CSBRouting::calcContactWeight(const L3Address& nextHop){
    std::string addr = nextHop.str();
    std::string strIndex = addr.substr(10,1);
    int index = stoi(strIndex);

    /*index - 1 because of the IP address number*/

    //in case that a contact message did not arrive before the route reply
    if(contactInfo[index-1]==nullptr)
        return 0;
    simtime_t contact = contactInfo[index-1]->getContactDuration();
    double contactTime = SIMTIME_DBL(contact);
    double contactNum = contactInfo[index-1]->getContactNumber();

    return contactTime/contactNum;
}

void CSBRouting::handleBlackListTimer()
{
