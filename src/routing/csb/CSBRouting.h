...
#ifndef __INET_CSBROUTING_H
#define __INET_CSBROUTING_H
...
#include "inet/routing/csb/CSBRouteData.h"
#include "inet/transportlayer/udp/UDPPacket.h"
#include "inet/routing/csb/CSBControlPackets_m.h"

#include "inet/common/geometry/common/Coord.h"
#include "inet/routing/csb/SocialNetwork.h"
#include "inet/networklayer/csbinfo/CSBContactInfo.h"

namespace inet {

/*
 * This class implements CSB routing protocol and Netfilter hooks
 * in the IP-layer required by this protocol.
 */

class INET_API CSBRouting : public cSimpleModule, public ILifecycle, public INetfilter::IHook, public cListener
{
private:
  protected:
    /*
    ...
    std::vector<CSBContactInfo*> contactInfo;
    ...
    // CSB parameters: the following parameters are configurable, see the NED file for more info.
    unsigned int rerrRatelimit = 0;
    unsigned int csbUDPPort = 0;
  ...
  protected:
    ...
    CSBRREPACK *createRREPACK();
    CSBRREP *createHelloMessage();
    CSBRREQ *createRREQ(const L3Address& destAddr);
    CSBRREP *createRREP(CSBRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& sourceAddr);
    CSBRREP *createGratuitousRREP(CSBRREQ *rreq, IRoute *originatorRoute);
    CSBRERR *createRERR(const std::vector<UnreachableNode>& unreachableNodes);
    ...
    void handleRREP(CSBRREP *rrep, const L3Address& sourceAddr);
    void handleRREQ(CSBRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive);
    void handleRERR(CSBRERR *rerr, const L3Address& sourceAddr);
    void handleHelloMessage(CSBRREP *helloMessage);
    void handleRREPACK(CSBRREPACK *rrepACK, const L3Address& neighborAddr);
    void handleCONTACTINFO(CONTACTINFO *contactInfoMsg);

    /*Contact*/
    void initializeContactTable();
    void updateContactInfo(CSBContactInfo *cInfo, L3Address& sourceAddr);
    ...
    void sendRREQ(CSBRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive);
    void sendRREPACK(CSBRREPACK *rrepACK, const L3Address& destAddr);
    void sendRREP(CSBRREP *rrep, const L3Address& destAddr, unsigned int timeToLive);
    void sendGRREP(CSBRREP *grrep, const L3Address& destAddr, unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREP(CSBRREP *rrep, const L3Address& destAddr, unsigned int timeToLive);
    void forwardRREQ(CSBRREQ *rreq, unsigned int timeToLive);
    ...
    void sendCSBPacket(CSBControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay);
    void clearState();
