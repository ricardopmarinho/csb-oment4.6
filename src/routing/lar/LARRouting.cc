...
#include "inet/routing/lar/LARRouting.h"
...
Define_Module(LARRouting);

void LARRouting::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        numHosts = (int)host->getParentModule()->par("numHosts").doubleValue();
        createNodeInfoVet();

        larUDPPort = par("udpPort");
...
void LARRouting::handleMessage(cMessage *msg)
{
   ...
   if (!isOperational) {
    ...
    if (msg->isSelfMessage()) {
        if (dynamic_cast<WaitForRREP *>(msg))
            ...
            rreqCount = rerrCount = 0;
            scheduleAt(simTime() + 1, counterTimer);
            MOVEINFO* moveMsg = createMOVEINFO(host->getIndex(),baseMobility->getLastPosition().x,baseMobility->getLastPosition().y,baseMobility->getMaxSpeed());
            sendLARPacket(moveMsg,addressType->getBroadcastAddress(),1,0);
        }
        ...
        if(strncmp("ICMP",msg->getName(),strlen("ICMP"))){
            ...
            LARControlPacket *ctrlPacket = check_and_cast<LARControlPacket *>(udpPacket->decapsulate());
            ...
            switch (ctrlPacket->getPacketType()) {
                case RREQ:
                    ...
                    handleRREQ(check_and_cast<LARRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                    break;

                case RREP:
                    rrepRec++;
                    emit(rrepRecSignal,rrepRec);
                    handleRREP(check_and_cast<LARRREP *>(ctrlPacket), sourceAddr);
                    break;

                case RERR:
                    handleRERR(check_and_cast<LARRERR *>(ctrlPacket), sourceAddr);
                    break;

                case RREPACK:
                    handleRREPACK(check_and_cast<LARRREPACK *>(ctrlPacket), sourceAddr);
                    break;

                case MoveInfo:
                    handleMOVEINFO(check_and_cast<MOVEINFO*>(ctrlPacket));
                    break;

                default:
                    throw cRuntimeError("LAR Control Packet arrived with undefined packet type: %d", ctrlPacket->getPacketType());
            }
            delete udpPacket;
        }
    }
...
INetfilter::IHook::Result LARRouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        LARRouteData *routeData = route ? dynamic_cast<LARRouteData *>(route->getProtocolData()) : nullptr;
...
LARRouting::LARRouting()
{
}

bool LARRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void LARRouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
    ...
    if(nodesInfo[hostIndex-1] == nullptr)
        return;

    hostIndex -= 1;
    unsigned int type = 0;
    Coord senderCoord(Coord::ZERO);
    senderCoord.x = baseMobility->getLastPosition().x;
    senderCoord.y = baseMobility->getLastPosition().y;
    senderCoord.z = 0;
    if(baseMobility->getLastPosition().x < nodesInfo[hostIndex]->getXPosition()){
        if(baseMobility->getLastPosition().y < nodesInfo[hostIndex]->getYPosition())
            type = 1;
        else
            type = 2;
    }else{
    ...
    Coord receiverPossibleCoord = createReceiverCoord(nodesInfo[hostIndex]->getXPosition(),nodesInfo[hostIndex]->getYPosition(),nodesInfo[hostIndex]->getSpeed(),nodesInfo[hostIndex]->getTime(),type);

    LARRREQ *rreq = createRREQ(target,senderCoord,receiverPossibleCoord,type);
    addressToRreqRetries[target] = 0;
    sendRREQ(rreq, addressType->getBroadcastAddress(), timeToLive);
}

L3Address LARRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void LARRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void LARRouting::sendRREQ(LARRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendLARPacket(rreq, destAddr, timeToLive, jitterPar->doubleValue());
    rreqCount++;
    rreqSent++;
    emit(rreqSentSignal,rreqSent);
}

void LARRouting::sendRREP(LARRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    LARRouteData *destRouteData = check_and_cast<LARRouteData *>(destRoute->getProtocolData());
    ...
    sendLARPacket(rrep, nextHop, timeToLive, 0);
    rrepSent++;
    emit(rrepSentSignal,rrepSent);
}

LARRREQ *LARRouting::createRREQ(const L3Address& destAddr, Coord senderCord, Coord receiverCord,unsigned int type)
{
    LARRREQ *rreqPacket = new LARRREQ("LAR-RREQ");
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        LARRouteData *routeData = check_and_cast<LARRouteData *>(lastKnownRoute->getProtocolData());
    ...
    rreqPacket->setSenderCoord(senderCord);
    rreqPacket->setReceiverPossibleCoord(receiverCord);
    rreqPacket->setType(type);

    return rreqPacket;
}

LARRREP *LARRouting::createRREP(LARRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    LARRREP *rrep = new LARRREP("LAR-RREP");
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // node is itself the requested destination
        ...
        LARRouteData *destRouteData = check_and_cast<LARRouteData *>(destRoute->getProtocolData());
        LARRouteData *originatorRouteData = check_and_cast<LARRouteData *>(originatorRoute->getProtocolData());
...
MOVEINFO *LARRouting::createMOVEINFO(unsigned int hostIndex, double x, double y, long speed){

    char msgName[20];
    sprintf(msgName,"MOVEINFO");

    MOVEINFO* moveMsg = new MOVEINFO(msgName);
    moveMsg->setX(x);
    moveMsg->setY(y);
    moveMsg->setSpeed(speed);
    moveMsg->setHostIndex(hostIndex);

    return moveMsg;

}

LARRREP *LARRouting::createGratuitousRREP(LARRREQ *rreq, IRoute *originatorRoute)
{
    ASSERT(originatorRoute != nullptr);
    LARRREP *grrep = new LARRREP("LAR-GRREP");
    LARRouteData *routeData = check_and_cast<LARRouteData *>(originatorRoute->getProtocolData());
...
void LARRouting::handleRREP(LARRREP *rrep, const L3Address& sourceAddr)
{
    EV_INFO << "LAR Route Reply arrived with source addr: " << sourceAddr << " originator addr: " << rrep->getOriginatorAddr()
    ...
    LARRouteData *destRouteData = nullptr;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<LARRouteData *>(destRoute->getProtocolData());
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (rrep->getAckRequiredFlag()) {
                    LARRREPACK *rrepACK = createRREPACK();
                ...
                if (nextHopToDestRoute && nextHopToDestRoute->getSource() == this) {
                    LARRouteData *nextHopToDestRouteData = check_and_cast<LARRouteData *>(nextHopToDestRoute->getProtocolData());
                    nextHopToDestRouteData->addPrecursor(originatorRoute->getNextHopAsGeneric());
                }
                LARRREP *outgoingRREP = rrep->dup();
...
void LARRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    LARRouteData *routingData = check_and_cast<LARRouteData *>(route->getProtocolData());
...
void LARRouting::sendLARPacket(LARControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
    ...
    udpPacket->setSourcePort(larUDPPort);
    udpPacket->setDestinationPort(larUDPPort);
...
void LARRouting::handleMOVEINFO(MOVEINFO *moveMsg){

    //stores information about positioning from other hosts

    int hostIndex = moveMsg->getHostIndex();
    nodesInfo[hostIndex] = new LARNodeInfo(moveMsg->getX(),moveMsg->getY(),moveMsg->getSpeed(),simTime(),hostIndex);
    /*EV << "MOVEINFO[" << hostIndex << "] message infos: " << nodesInfo[hostIndex]->getXPosition() << ", " << nodesInfo[hostIndex]->getYPosition() << ", " << nodesInfo[hostIndex]->getSpeed() << ", "
            << nodesInfo[hostIndex]->getIndex() << endl;*/

}

void LARRouting::createNodeInfoVet(){

    int i;

    for(i=0;i<numHosts;i++)
        nodesInfo.push_back(nullptr);

    /*if(nodesInfo == nullptr){
        nodesInfo = (LARNodeInfo**)malloc((index + 1)*sizeof(LARNodeInfo*));
        numHosts = index+1;
        for(int i = 0; i < numHosts; i++)
            nodesInfo[i] = nullptr;
    }else if(index >= numHosts){
        LARNodeInfo **aux = nodesInfo;
        nodesInfo = (LARNodeInfo**)malloc((index + 1)*sizeof(LARNodeInfo*));
        for(int i = 0; i < numHosts; i++){
            if(aux[i] != nullptr)
                nodesInfo[i] = new LARNodeInfo(aux[i]->getXPosition(),aux[i]->getYPosition(),aux[i]->getSpeed(),aux[i]->getTime(),index);
        }
        delete(aux);
    }*/
}

void LARRouting::handleRREQ(LARRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    EV_INFO << "LAR Route Request arrived with source addr: " << sourceAddr << " originator addr: " << rreq->getOriginatorAddr()
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        LARRouteData *routeData = check_and_cast<LARRouteData *>(reverseRoute->getProtocolData());
    ...
    LARRouteData *destRouteData = destRoute ? dynamic_cast<LARRouteData *>(destRoute->getProtocolData()) : nullptr;
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // create RREP
        LARRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
        ...
        destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag()) {
            ...
            LARRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        LARRREQ *outgoingRREQ = rreq->dup();
        forwardRREQ(outgoingRREQ, timeToLive);
    }
    else
        EV_WARN << "Can't forward the RREQ because of its small (<= 1) TTL: " << timeToLive << " or the LAR reboot has not completed yet" << endl;

    delete rreq;
}

bool LARRouting::hostCanForward(LARRREQ *rreq){
    int type = rreq->getType();
    Coord senderCoord = rreq->getSenderCoord();
    Coord receiverCoord = rreq->getReceiverPossibleCoord();
    switch(type){
    case 1:
        if((baseMobility->getLastPosition().x > senderCoord.x && baseMobility->getLastPosition().x < receiverCoord.x)
                && (baseMobility->getLastPosition().y > senderCoord.y && baseMobility->getLastPosition().y < receiverCoord.y))
            return true;
    case 2:
        if((baseMobility->getLastPosition().x > senderCoord.x && baseMobility->getLastPosition().x < receiverCoord.x)
                && (baseMobility->getLastPosition().y < senderCoord.y && baseMobility->getLastPosition().y > receiverCoord.y))
...
IRoute *LARRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    LARRouteData *newProtocolData = new LARRouteData();
    ...
    newRoute->setSourceType(IRoute::LAR);
...
void LARRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
...
void LARRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    LARRouteData *unreachableRouteData = check_and_cast<LARRouteData *>(unreachableRoute->getProtocolData());
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getNextHopAsGeneric() == unreachableAddr) {
            LARRouteData *routeData = check_and_cast<LARRouteData *>(route->getProtocolData());
    ...
    LARRERR *rerr = createRERR(unreachableNodes);
    rerrCount++;

    // broadcast
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendLARPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());
}

LARRERR *LARRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    LARRERR *rerr = new LARRERR("LAR-RERR");
...
Coord LARRouting::createReceiverCoord(double x,double y,long speed,simtime_t time, unsigned int type){
...
void LARRouting::handleRERR(LARRERR *rerr, const L3Address& sourceAddr)
{
    EV_INFO << "LAR Route Error arrived with source addr: " << sourceAddr << endl;
        ...
        if (route->getNextHopAsGeneric() == sourceAddr) {
            for (unsigned int j = 0; j < unreachableArraySize; j++) {
                if (route->getDestinationAsGeneric() == rerr->getUnreachableNodes(j).addr) {
                    ...
                    routeData->setIsActive(false);    // it means invalid, see 3. LAR Terminology p.3. in RFC 3561
                    routeData->setLifeTime(simTime() + deletePeriod);
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        LARRERR *newRERR = createRERR(unreachableNeighbors);
        sendLARPacket(newRERR, addressType->getBroadcastAddress(), 1, 0);
        rerrCount++;
    }
    delete rerr;
}

bool LARRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void LARRouting::handleWaitForRREP(WaitForRREP *rrepTimer)
{
    ...
    LARRREQ *rreq = createRREQ(destAddr,senderCoord,receiverPossibleCoord,type);
...
void LARRouting::forwardRREP(LARRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendLARPacket(rrep, destAddr, 100, jitterPar->doubleValue());
}

void LARRouting::forwardRREQ(LARRREQ *rreq, unsigned int timeToLive)
{
    EV_INFO << "Forwarding the Route Request message with TTL= " << timeToLive << endl;
    sendLARPacket(rreq, addressType->getBroadcastAddress(), timeToLive, jitterPar->doubleValue());
}

void LARRouting::completeRouteDiscovery(const L3Address& target)
{
...
void LARRouting::sendGRREP(LARRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendLARPacket(grrep, nextHop, timeToLive, 0);
}

LARRREP *LARRouting::createHelloMessage()
{
    ...
    LARRREP *helloMessage = new LARRREP("LAR-HelloMsg");
...
void LARRouting::sendHelloMessagesIfNeeded()
{
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval)) {
        ...
        LARRREP *helloMessage = createHelloMessage();
        sendLARPacket(helloMessage, addressType->getBroadcastAddress(), 1, 0);
    }

    scheduleAt(simTime() + helloInterval - periodicJitter->doubleValue(), helloMsgTimer);
}

void LARRouting::handleHelloMessage(LARRREP *helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        LARRouteData *routeData = check_and_cast<LARRouteData *>(routeHelloOriginator->getProtocolData());
...
void LARRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result LARRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
    ...
    LARRouteData *routeDestData = routeDest ? dynamic_cast<LARRouteData *>(routeDest->getProtocolData()) : nullptr;
...
void LARRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IRoute *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    LARRouteData *unreachableRouteData = unreachableRoute ? dynamic_cast<LARRouteData *>(unreachableRoute->getProtocolData()) : nullptr;
    ...
    rerrCount++;
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendLARPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());    // TODO: unicast if there exists a route to the source
}

void LARRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool LARRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        LARRouteData *routeData = check_and_cast<LARRouteData *>(route->getProtocolData());
...
LARRREPACK *LARRouting::createRREPACK()
{
    LARRREPACK *rrepACK = new LARRREPACK("LAR-RREPACK");
    rrepACK->setPacketType(RREPACK);
    rrepACK->setByteLength(2);
    return rrepACK;
}

void LARRouting::sendRREPACK(LARRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendLARPacket(rrepACK, destAddr, 100, 0);
}

void LARRouting::handleRREPACK(LARRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void LARRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void LARRouting::handleBlackListTimer()
{
